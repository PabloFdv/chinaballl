...
function recalculateRoomPingBalance(room) {
...
app.post("/api/rooms", (req, res) => {
  ...
  const validMapSize = ["1v1", "2v2", "3v3", "4v4"].includes(mapSize) ? mapSize : `${validTeamSize}v${validTeamSize}`;
  const clientIp = req.headers["x-forwarded-for"]?.split(",")[0]?.trim() || req.socket.remoteAddress || "127.0.0.1";
  const maskedIp = clientIp.includes(".") ? clientIp.split(".").slice(0, 2).join(".") + ".*.*" : "Local-IP";
  const id = `sala-${Date.now()}-${Math.floor(Math.random() * 1e3)}`;
  const newRoom = {
    id,
...
wss.on("connection", (ws, req) => {
  ...
  const playerIp = rawIp.includes(".") ? rawIp.split(".").slice(0, 2).join(".") + ".*.*" : "127.0.*.*";
  ws.on("message", (data) => {
    try {
      ...
      if (msg.type === "join_room") {
        ...
        let assignedTeam = msg.preferredTeam;
          ...
          if (sender && (sender.isHost || sender.isReferee)) {
            ...
            if (target) {
              ...
              broadcastToRoom(room, {
                ...
                message: `\xC1rbitro transferido para ${target.name}!`
              });
            }
          }
        }
          ...
          if (sender && (sender.isHost || sender.isReferee)) {
            ...
            if (redCount === 0 || blueCount === 0) {
              ws.send(JSON.stringify({
                type: "room_notice",
                message: "Para iniciar a partida online \xE9 necess\xE1rio pelo menos 1 jogador real em cada time! Convide um amigo pelo link."
              }));
              return;
            }
            ...
            broadcastToRoom(room, {
              ...
              message: `Partida iniciada pelo \xE1rbitro ${sender.name}!`
            });
          }
        }
      }
        ...
        if (room) {
          ...
          if (sender && (sender.isHost || sender.isReferee)) {
            ...
            broadcastToRoom(room, {
              ...
              message: "Lados invertidos pelo \xE1rbitro (Vermelho \u21C4 Azul)!"
            });
          }
        }
      }
        ...
        if (room) {
          ...
          if (sender && (sender.isHost || sender.isReferee)) {
            ...
            broadcastToRoom(room, {
              ...
              message: "Times sorteados aleatoriamente pelo \xE1rbitro!"
            });
          }
        }
      }
          ...
          if (wasHost && room.players.size > 0) {
            ...
            if (newHost) {
              ...
              broadcastToRoom(room, {
                ...
                isReferee: true,
                message: `${newHost.name} assumiu a lideran\xE7a e arbitragem da sala!`
              });
            }
          }
...
function broadcastToRoom(room, msg, excludePlayerId) {
